...
import { EDIT_STORE_RESET } from "../../../constants/ecommerceConstants";
...
const EditStore = () => {
  ...
  const { loading, editStore, error, store } = useSelector(
    (state) => state.store || []
  );
  ...
  const submitStoreHandler = (e) => {
    ...
    // }
      ...
      dispatch(editStoreAction(storeFormData, id));
    }
  };

  useEffect(() => {
    if (editStore?.success) {
      gaEventTracker("Ecommerce", "User updated store");
  ...
  }, [navigate, dispatch, error, editStore, pathname]);
